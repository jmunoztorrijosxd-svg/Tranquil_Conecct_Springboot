...
public String mostrarpagina(
    ...
    if (userDetails != null) {
        ...
        usuarioLogueado = usuarioRepository
                .findByCorreo(correo)
                .orElse(null);

        if (usuarioLogueado != null) {
            List<Cita> misCitas =
                    citaRepository.findByPaciente(usuarioLogueado);
    ...
    List<Usuario> listaPsicologos =
            usuarioRepository.findByRolAndEstadoValidacion(
                    "PSICOLOGO",
                    "APROBADO");
...
public String guardarCita(HttpSession session,
                           Authentication auth) {
    ...
    if (psicologoId == null || fechaStr == null) {
    ...
    // Limpiar la sesión
    session.removeAttribute("psicologoId");
