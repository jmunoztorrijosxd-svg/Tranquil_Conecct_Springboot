...
public class RegistroPsicologoController {
    ...
    @Autowired
    private com.tranquilconnect.tranquilconnect.repository.CitaRepository citaRepository;
    ...
    public String mostrarDashboardPsicologo(Authentication authentication, Model model) {
        ...
        model.addAttribute("usuario", usuario);
        // Cargar pacientes con los que el psicólogo tuvo citas
        java.util.List<com.tranquilconnect.tranquilconnect.model.Cita> citas = citaRepository.findByPsicologo(usuario);
        java.util.List<com.tranquilconnect.tranquilconnect.model.Usuario> pacientes = citas.stream().map(c -> c.getPaciente()).distinct().collect(java.util.stream.Collectors.toList());
